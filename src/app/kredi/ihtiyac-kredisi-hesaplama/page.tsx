  ...
  calculator: {
    ...
    calculate: async (inputs: { [key:string]: string | number | boolean }): Promise<CalculationResult | null> => {
        ...
        if (isNaN(loanAmount) || loanAmount <= 0) {
            return { summary: { error: { error: { label: 'Hata', value: 'Lütfen geçerli bir kredi tutarı girin.' } } } };
        }
        if (isNaN(monthlyInterestRate) || monthlyInterestRate <= 0) {
            return { summary: { error: { error: { label: 'Hata', value: 'Lütfen geçerli bir faiz oranı girin.' } } } };
        }
        if (isNaN(loanTerm) || loanTerm <= 0) {
            return { summary: { error: { error: { label: 'Hata', value: 'Lütfen geçerli bir vade süresi girin.' } } } };
        }
        ...
        const summary: CalculationResult['summary'] = {
            ...
            loanAmount: { label: 'Kredi Tutarı', value: formatCurrency(loanAmount) },
        };
