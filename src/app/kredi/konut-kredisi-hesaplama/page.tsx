...
const pageConfig = {
  ...
  calculator: {
    ...
    calculate: async (inputs: { [key: string]: string | number | boolean }): Promise<CalculationResult | null> => {
        ...
        const monthlyInterestRate = Number(inputs.interestRate);

        if (isNaN(principal) || isNaN(termInYears) || isNaN(monthlyInterestRate) || principal <= 0 || termInYears <= 0 || monthlyInterestRate <= 0) {
            return { summary: { error: { error: { label: 'Hata', value: 'Lütfen tüm alanlara geçerli ve pozitif değerler girin.' } } } };
        }
        ...
        const summary = {
            monthlyPayment: { label: 'Aylık Taksit', value: formatCurrency(monthlyPayment), isHighlighted: true },
            totalPayment: { label: 'Toplam Geri Ödeme', value: formatCurrency(totalPayment) },
            totalInterest: { label: 'Toplam Faiz', value: formatCurrency(totalInterest) },
            loanAmount: { label: 'Kredi Tutarı', value: formatCurrency(principal) },
        };
